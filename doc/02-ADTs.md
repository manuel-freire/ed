...
This will repeat a **lot** of work\footnote{if you set the initial value of \texttt{life} too high; and otherwise, you may get a wrong answer!}. And what should I initialize **`life`** to?

~~~{.cpp}
bool isHappy(int n, int life) {
    if (n == 1) return true;     // happy
    if (life == 0) return false; // unhappy? depends!
...
This does **not** duplicate work\footnote{because it can rely on an efficient \texttt{set} ADT that tracks known numbers, allowing it to stop as soon as the 1st duplicate is found, which indicates a loop}. And you do not need to initialize **`life`** at all!

~~~{.cpp}
bool isHappy2(int n, std::set<int> &known) {
    if (n == 1) return true;         // happy
    if ( ! known.insert(n).second) { // unhappy: loop found
        return false;
    }
    int sum = 0;
    for (/**/; n > 0; n/=10) {
...
- **observers**, for safe read-only access to day, dayOfWeek, month and year
- a **distance** operation that returns days
- **constructors**, both full and offset

Have a look at real examples\footnote{Dates are \emph{hard}. Be prepared to learn a lot about humans and time-keeping. Both Java \& JavaScript have extensively rewritten theirs.} to see how dates are handled!

- [Java JodaTime](https://www.joda.org/joda-time/apidocs/index.html)
- [Java 11 LocalDate](https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/time/LocalDate.html)
...
- \textbf{convention}: if there is no other mechanism, then the user and the implementer may agree in not to use or modify whatever is marked as private. A typical mark is to use **`_`** as the first character of an identifier, such as `_day`. It is not necessary in C++ (although it is useful), but it was in C, where there is no compiler-enforced privacy.
...
- ... but *the ADT user would not be able to see the difference*, since the public part would be the same. 
- Designing a good ADT must carefully consider *exactly how* that ADT is going to be used, and 
    + make the public part easy to understand and use right
    + make the private part correct & efficient at supporting the public contract
...
In C++, this involves the \emph{declaration} of a class in a .h header file (e.g. \emph{date.h}), and its \emph{implementation} in an associated .cpp file (e.g. \emph{date.cpp}), with the h-file protected using include-guards\footnote{Yes, it is 2026 and we still do this. Yes, C++ is, language-wise, quite dated. When we adopt C++ 20 we can get rid of them... except for ~50 years of legacy code}:
...
## ADTs as modules in the C++ Standard Library

~~~{.cpp}
#include <iostream>  // <-- many modules have a lot more than 1 ADT
#include <set>       // <-- but for containers, each ADT is generally 1 header
...
(from https://en.cppreference.com/w/cpp/language/modules.html)
...
- In this section we analyze how to code ADTs in C++, which is a very flexible and powerful language\footnote{but also crufty and dangerous. We should really be teaching \textbf{Rust} }. In general, there is more than one way to do something, and the disadvantages of certain implementations may not be clear.
...
The **const** modifier *should* be used whenever possible\footnote{In this subject, you \emph{will} get lower grades if you break this rule}, as it provides several advantages. For example, an object without mutators can be shared without running the risk of having the object modified by third parties.
...
- an \emph{abstraction function}: how we interpret values of the representing types. For example, as seconds since 1970-1-1 at 0:00
- an \emph{equivalence function}: conditions that must be met for two ADT values to be considerd "the same". 
...
- there is no "minimum" of an empty array of numbers
- ...
...
They, too, are considered *partial*. 

## Partiality and errors

For practical purposes, if it *can* error out with any conceivable ADT state and/or parameters, an operation is **partial**

- You normally use documentation comments to warn about partiality. 
- And then, if the warning is not heeded, you need to signal an *error*.

So, how do we notify users that an **error** (due to *partiality*) has occured? There are several options...

## Communicating errors

- Return an **error value**. For example, in `indexOf(E element)`, you could return an invalid index, `-1`.

    + you need to have a **clearly invalid** value to return as an error! What if you were implementing `add(int x, int y)`?
    + the user needs to **remember to check** the return value. Example: `mySet.remove(value)` in Java returns a boolean if removal failed. Would you remember to check it? Should you?
  
- Set an **error flag**, which is some kind of global variable (eg.: `errno`) that must be checked to see if there was an error or not (and possibly an integer error-code\footnote{see, for example, a linux manual page: \url{https://linux.die.net/man/3/connect} }).

    + Mitigates "must be able to return a clearly invalid value" problem, but is *very* easy to ignore.
...
- Use **a return-type that can handle both** normal and error conditions, so that the caller can tell both apart and access whichever applies.
...
~~~{.rust}
pub enum Result<T, E> { // part of the standard library of types
    Ok(T),
    Err(E),
}
...
* Java exceptions must be `Throwable`. This base exception class has ... 
    - an optional cause (another exception that caused *this* exception to be thrown)
    - a full stack-trace
...
C++ exceptions are similar to Java `run-time` exceptions, and can be of any type, so you cannot (without writing a dedicated exception class), rely on all the "free" machinery that Java exceptions have. 

- - - 

Since C++ 11, you do have a [`noexcept`](https://en.cppreference.com/w/cpp/language/noexcept_spec) modifier, which is handy to indicate that you *cannot* throw. You can also query the compiler to know if something can conceivably throw:
...
if (noexcept(no_throw()) {
    ...
    // Since C++ 17, there is no need to write "noexept",
    // the compiler generates "noexcept" for you (if it is sure)
}
~~~

We will not be using this, but it is good to know it exists.

# Pointers, references and return-types
...
- Accessing an **un-allocated** pointer, or accessing an offset of that pointer that is **beyond the allocated size**. Will generally contain rubbish, and probably cause the program to crash. Attempting to de-reference `NULL` or `nullptr` (=0) in particular should always crash the program.
...
-  Implement the \texttt{Complex} ADT that represents complex numbers in C++. Use \texttt{float} for the real and imaginary parts of the numbers, with operations for adding, subtracting, multiplying and calculating the module of the number, along with observer methods that return the real and imaginary parts of the number.
